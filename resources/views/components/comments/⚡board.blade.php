        ...
        <div class="flex justify-between">
            ...
            <div class="flex items-center justify-center gap-6">
                ...
                <div class="relative inline-block text-left">
                    ...
                    >
                        <x-icons.animate-spin class="size-5" wire:loading wire:target="changeOrder" />
                        <x-icons.filter-left class="size-5" wire:loading.remove wire:target="changeOrder" />
                        <span>{{ __('Sort by') }}</span>
                    </button>

                    <div
                        class="absolute right-0 z-10 mt-2 w-32 origin-top-right rounded-md bg-zinc-50 shadow-lg ring-1 ring-black/5 focus:outline-hidden dark:bg-zinc-800 dark:ring-white/20"
                            ...
                            @foreach (CommentOrderOptions::cases() as $commentOrder)
                                <button
                                    ...
                                    @class([
                                        ...
                                        'bg-zinc-200 text-zinc-900 outline-hidden dark:bg-zinc-600 dark:text-zinc-50' => $order === $commentOrder,
                                        'text-zinc-700 hover:bg-zinc-100 dark:text-zinc-400 dark:hover:bg-zinc-700' => $order !== $commentOrder,
                                    ])
            ...
            <button
                class="before:bg-lividus-600 group dark:bg-lividus-700 relative transform-[translateZ(0)] cursor-pointer overflow-hidden rounded-xl bg-emerald-600 px-6 py-2 before:absolute before:bottom-0 before:left-0 before:h-full before:w-full before:origin-bottom-right before:scale-x-0 before:transition before:duration-500 before:ease-in-out hover:before:origin-top-left hover:before:scale-x-100 dark:before:bg-emerald-700"
                type="button"
                x-on:click="
                    $dispatch('open-create-comment-modal', {
                        parentId: null,
                        replyTo: '',
                    })
                "
